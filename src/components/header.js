// No necesitamos importar nombres de íconos específicos aquí
// ya que asumimos que lucide se carga globalmente como antes.
// import { createIcons } from 'lucide'; // Esta línea no es necesaria si usas la carga global <script>
...
const headerHTML = `
  <div class="container mx-auto px-6 py-4 flex justify-between items-center">
    ...
    <nav class="hidden md:flex space-x-8 items-center">
      ...
      <button id="nav-cart-btn" aria-label="Ver carrito de compras" class="relative text-gray-600 hover:text-pink-500">
        ...
        <!-- Añadido aria-hidden para que lectores de pantalla no lean el número dos veces -->
        <span id="cart-count-badge" class="absolute -top-2 -right-2 bg-pink-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center" aria-hidden="true">0</span>
      </button>
    </nav>
    <!-- --- AÑADIDO: aria-label --- -->
  ...
  <!-- Menú móvil sin cambios en el HTML -->
...
export const renderHeader = (container, onNavigate, onCartClick) => {
  ...
  // --- Event Listeners (sin cambios funcionales, solo se añade lógica aria-label) ---
  ...
  document.getElementById('mobile-nav-about').addEventListener('click', (e) => { e.preventDefault(); onNavigate('about'); mobileMenu.classList.add('hidden'); });

  const mobileMenuBtn = document.getElementById('mobile-menu-btn');
  mobileMenuBtn.addEventListener('click', () => {
    mobileMenu.classList.toggle('hidden');
    // --- Lógica para actualizar aria-label del botón menú ---
    const isOpen = !mobileMenu.classList.contains('hidden');
    mobileMenuBtn.setAttribute('aria-label', isOpen ? 'Cerrar menú de navegación' : 'Abrir menú de navegación');
  });

  // Ya no necesitamos llamar a createIcons aquí si usamos la carga global
  // createIcons();
};

export const updateHeaderCartCount = (count) => {
  const badge = document.getElementById('cart-count-badge');
  const cartBtn = document.getElementById('nav-cart-btn'); // Referencia al botón
  if (badge && cartBtn) {
    badge.textContent = count;
    // --- Lógica para actualizar aria-label del botón carrito ---
    if (count > 0) {
      cartBtn.setAttribute('aria-label', `Ver carrito de compras (${count} ${count === 1 ? 'artículo' : 'artículos'})`);
    } else {
      cartBtn.setAttribute('aria-label', 'Ver carrito de compras (vacío)');
    }
  }
};
